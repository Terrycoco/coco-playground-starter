...
export default function Scale() {
  ...
  const generate = () => {
    ...
    if (levels.hasOwnProperty(prev) && firstRatio != null) {
      ...
      if (!levs.hasOwnProperty(next)) {
        levs[next] = {};
        levs[next].fontSize = parseInt(suggested); //suggest a size?
  ...
  const getParaStyle = () => {
    if (levels[device].hasOwnProperty("base")) {
      let style = {
        fontFamily: getFontVariable(theme.fonts.base),
        fontSize: levels[device].base.fontSize + "px",
        lineHeight: levels[device].base.lineHeight,
        marginBottom: "1rem",
      };
      return style;
    }
  ...
  const handleFontSizeChange = (levelname, newval) => {
    ...
    //set latest ratio
    if (levelname === "fs1") {
      let bfs = levels[device].base.fontSize;
