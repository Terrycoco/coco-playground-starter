...
const DropdownOption = (props) => {
  const handleSelect = (e) => {
    props.onSelect({key: e.target.id, value: props.value, label: props.label});
  };
...
const FontDropdown = ({ options, defaultObj, onSelect, ...props }) => {
  ...
  const setCurrentObj = useCallback(() => {
  ...
  const handleSelect = (newval, newlabel) => {
    ...
    onSelect({ value: newval, label: newlabel, key: props. }); //to parent
    setIsOpen(false);
  };
