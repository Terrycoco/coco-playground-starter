...
const ColorForm = (props) => {
  ...
  const containers = useSelector(containersSlice);
      ...
      {tab == 1 && (
        <FormWrapper>
          <StyleGrid title="header">
            <StyleGridItem label="backgroundColor" key={Math.random()}>
              <ColorDropdown
                key={Math.random()}
                element={"header"}
                propName="backgroundColor"
                id="hbgcolor"
                defaultValue={containers.header.backgroundColor} //should be var(--clr-primary)
