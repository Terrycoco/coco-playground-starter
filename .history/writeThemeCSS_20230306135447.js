...
for (const device in theme.screens) {
  ...
  for (const level in theme.fontSizes[device]) {
    let last = level.slice(-1);
    if (level === "body") {
      css += `--fs${level}: ${theme.fontSizes[device][level].fontSize}px;\n`;
      css += `--lh${level}: ${theme.fontSizes[device][level].lineHeight};\n`;
      css += `--fs0: ${theme.fontSizes.mobile[level].fontSize}px;\n`;
      css += `--lh0: ${theme.fontSizes.mobile[level].lineHeight};\n`;
    } else {
      css += `--fs${last}: ${theme.fontSizes[device][level].fontSize}px;\n`;
      css += `--lh${last}: ${theme.fontSizes[device][level].lineHeight};\n`;
    }
  }
  css += `}\n`;
  css += `}\n\n`;
