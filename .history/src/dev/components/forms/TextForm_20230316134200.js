...
const SliderItem = ({ element, propName, defaultValue, onSelect }) => {
  ...
  return (
    <StyleGridItem label={propName} key={useId}>
      <RangeSlider
        key={useId}
...
const TextForm = (props) => {
      ...
      Object.keys(text[el]).map((prop, idx) => {
        ...
        if (prop.includes("fontSize") || prop.includes("lineheight")) {
          ...
          if (thisObj !== undefined) {
            ...
            result.push(
              <StyleGridItem label={prop} key={useId}>
                <FontDropdown
                  key={propkey}
                  element={el}
                  propName={prop}
          ...
          result.push(
            <StyleGridItem label={prop} key={`${prop}${useId}`}>
              <ColorDropdown
                key={propkey}
                element={el}
                propName={prop}
  ...
  return (
    <FormWrapper>
      ...
      >
        Remember: fontSize and lineHeight are set in Scale
      </div>
      {getElements()}
    </FormWrapper>
  );
