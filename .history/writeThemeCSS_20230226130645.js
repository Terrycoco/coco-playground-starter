...
function sectionToCSSVariables(themeobj, sectionname, prefix) {
  let result = "";
  let value = "";
...
prefix = "--font-";
css += sectionToCSSVariables(theme, "fonts", prefix);

//spacing variables
prefix = "--sp-";
css += sectionToCSSVariables(theme, "spacing", prefix);
...
for (const key in theme.text) {
  prefix = "";
  css += `${key}: {\n`;
  css += sectionToCSSVariable(theme.text, key, prefix);
  css += "}\n\n";
}
