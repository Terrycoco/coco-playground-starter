...
const TextForm = ({ styleObj, section, element, ...props }) => {
  ...
  const handleValueChange = (newval, id) => {
    // console.log("got to handleValueChange textform ");
    // console.log("send from slider: ", newval);
    // console.log("element is: ", element);
    // console.log("prop is: ", id);

    //send to parent
    //make new font variation string
    let fv = `"wdth" ${newval}, "wght" ${wght}, "slnt" ${slnt}, "ital" ${ital}, "grad" ${grad}`;
    setWght(newval);
    props.onChange(fv);
  };
