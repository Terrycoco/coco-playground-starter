...
export default function Scale() {
  ...
  const getParaStyle = () => {
    if (currentDevice !== undefined) {
      ...
      //all other settings change rem to em
      return style;
    }
  };
  ...
  const getHeadingStyle = (level) => {
    if (level !== undefined && currentDevice !== undefined) {
      ...
      //get currently set styles for this level
      let style = { ...text[level] };

      //set scale props
      style.fontFamily = variables["var(--font-display)"];
      style.fontSize = fontSizes[currentDevice][level].fontSize + "px";
      style.lineHeight = fontSizes[currentDevice][level].lineHeight;
      style.marginBottom = style.marginBottom.replace("rem", "em");
  ...
  const getHeadings = () => {
    if (headingCount) {
      let result = [];
      for (let i = headingCount; i > 0; i--) {
        let h = `h${i}`;
        result.unshift(
          <div
            key={h}
            style={getHeadingStyle(h)}
          >{`Heading ${h}  Adjust line height between these lines`}</div>
        );
      }
      return result;
    }
