...
const initialState = {
  ...
  headingCount: currentTheme.headingCount < 1 ? 1 : currentTheme.headingCount, //min 1
};
...
export const fontSizesSlice = createSlice({
  ...
  reducers: {
    ...
    updateHeadingCount(state, action) {
      state.headingCount = action.payload;
    },
...
export const selectAllDeviceSizes = (state) => state.fontSizes.all;
export const selectCurrentDeviceSizes = (state) => {
  let cd = state.fontSizes.currentDevice;
  return state.fontSizes.all[cd];
};
export const selectHeadingCount = (state) => state.fontSizes.headingCount;
