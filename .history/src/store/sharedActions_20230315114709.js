...
const updateColor = createAction("UPDATE_COLOR", function prepare(key) {
  ...
  if (key !== "error" && key !== "whitish") {
    varKeys.map((pct) => {
      ...
      payloadVal.push({ key: variantKey, value: val, isVariant: true });
    });
  }
  //push original one
  payloadVal.push({ key: key, value: newhex, isVariant: false });
  return { payload: payloadVal };
});

console.log(updateColor({ key: "primary", value: "#ffffff" }));
