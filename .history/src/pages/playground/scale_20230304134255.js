...
const FontLevel = ({ fontSize, lineHeight, levelName, ...props }) => {
  const handleLineHeightChange = (newval) => {
  ...
  //init
  const [fsSlider, fsSliderConfig] = useSlider(
    {
      min: 8,
      max: 72,
      value: 18,
  ...
  const [lhSlider, lhSliderConfig] = useSlider(
    {
      min: 0.8,
      max: 3,
      value: 1.5,
  ...
  return (
    <StyleGrid title={levelName}>
      ...
      <StyleGridItem label="Font Size">
        <RangeSlider {...fsSliderConfig} />
      </StyleGridItem>

      <StyleGridItem label="Line Height">
        <RangeSlider {...lhSliderConfig} />
      </StyleGridItem>
    </StyleGrid>
  );
};
