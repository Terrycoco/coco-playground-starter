...
export default function Scale() {
  ...
  const generate = () => {
    ...
    newtheme.fontSizes[device] = levs;
    setTheme(newtheme);
  ...
  const getParaStyle = () => {
    if (device && levels.hasOwnProperty(device)) {
      let style = {
        fontFamily: getFontVariable(theme.fonts.base),
        fontSize: levels[device].body.fontSize + "px",
        lineHeight: levels[device].body.lineHeight,
        marginBottom: "1rem",
      };
      return style;
    }
  ...
  const handleFontSizeChange = (levelname, newval) => {
    // console.log("recd from ", id, newval);
    let levs = Object.assign({}, levels);
    levs[device][levelname].fontSize = newval;
    setLevels(levs);

    //set latest ratio
    if (levelname === "fs1") {
      let bfs = levels[device].body.fontSize;
  ...
  const showLevels = () => {
    ...
    for (let i = 0; i < levelCount + 1; i++) {
      ...
      if (levels[device].hasOwnProperty(levelName)) {
                ...
                <div>
                  ...
                  <div style={styles.labelblock}>
                    <div style={styles.label}>
                      Body Font: {theme.fonts.base}
                    </div>
                    <div style={styles.label}>
                      Body Font Size: {theme.fontSizes.mobile.body.fontSize} +
                      "px"
                    </div>
                  </div>
                </div>
