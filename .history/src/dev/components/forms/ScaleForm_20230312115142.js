...
const ScaleForm = (props) => {
  ...
  const fonts = useSelector(selectFontObjs);
  ...
  const showLevels = () => {
    let result = [];
    let levelCount = Object.keys(fontSizes[currentDevice]).length;

    for (let i = 0; i < levelCount; i++) {
      let levelName = i == 0 ? "body" : `fs${i}`;
      let fs = fontSizes[currentDevice][levelName].fontSize;
      //   // console.log("fs to ", levelName, fs);
      result.unshift(
        <FontLevel
          key={`${currentDevice}${levelName}`}
          fontSize={fs}
          lineHeight={fontSizes[currentDevice][levelName].lineHeight}
          levelName={levelName}
