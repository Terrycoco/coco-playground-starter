...
import { getFontVariable, getFontNameFromVar } from "@/utils/fonts";
...
const FontDropdown = ({ section, element, isBaseLevel, ...props }) => {
  ...
  const setCurrent = useCallback(() => {
    let val = "var(--font-gelasio)";
    let label = "Gelasio";
    if (isBaseLevel == true && themedFonts !== undefined) {
  ...
  const handleSelect = (e) => {
    ...
    if (isBaseLevel) {
      dispatch(updateFont({ key: element, value: e.label }));
    } else {
      dispatch(
