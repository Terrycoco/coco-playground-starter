...
for (const level in fontSizes.mobile) {
  if (level === "body") {
    css += `fs0: --${theme.fontSizes.mobile.body.fontSize}px;\n`;
    css += `lh0: --${theme.fontSizes.mobile.body.lineHeight};\n`;
  } else {
...
for (const device in theme.screens) {
  ...
  for (const key in theme.fontSizes[device]) {
    if (key === "body") {
      continue;
    }
    css += "--";
