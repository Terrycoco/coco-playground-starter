...
const StyleForm = ({ section, element, ...props }) => {
  ...
  const handleBlur = (e) => {
    ...
    let property = e.target.dataset.propname;
    console.log("property to change:", property);
    // newtheme[section][element][e.target.getAttribute("data-id")] =
    //   e.target.value;
  ...
  const renderElement = () => {
    ...
    for (const p in styleObj) {
      console.log("p:", p, "val:", styleObj[p]);
      result.push(
