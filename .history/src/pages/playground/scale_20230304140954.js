...
const FontLevel = ({ fontSize, lineHeight, levelName, ...props }) => {
  ...
  function handleLineHeightChange(fs) {
    console.log("received on parent:", fs);
    setFsVal(fs);
    props.onLineHeightChange(levelName, fs);
  }

  function handleFontSizeChange(lh) {
    console.log("received on parent:", lh);
    setLhVal(lh);
    props.onFontSizeChange(levelName, lh);
  }
