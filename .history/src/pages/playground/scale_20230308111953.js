...
export default function Scale() {
  ...
  const setRatio = useCallback(() => {
    if (fontSizes.hasOwnProperty("fs1")) {
      let base = getValFromCSS(fontSizes.base.fontSize);
      let fs1 = getValFromCSS(fontSizes.fs1.fontSize);
      let ratio = fs1 / base;
  ...
  const deleteLastLevel = () => {
    //delete last level made?
    dispatch(deleteLevel("fs2"));
  };
  ...
  const getHeadingStyle = (level) => {
    let style = {
      ...
      lineHeight: currentSizes[device][level].lineHeight,
      fontWeight: "bold",
