...
const TextForm = ({ styleObj, section, element, ...props }) => {
  ...
  const handleValueChange = (newval, id) => {
    ...
    switch (id) {
      ...
      case "slnt": {
        setSlnt(newval);
        fv = ` 'wght' ${wght}, 'slnt' ${newval}`;
        props.onChange(fv);
        break;
      }
      case "ital": {
        setItal(newval);
        fv = ` 'wght' ${wght}, 'ital' ${newval}`;
        props.onChange(fv);
        break;
      }
    }
  ...
  return thisSO ? (
    <>
      <div style={styles.form} onClick={handleClick}>
        ...
        <div key={"slant-row"} style={styles.row}>
          <div style={styles.prop}>slant</div>
          <div style={styles.val}>
            <Slider
              id="slnt"
              min="0"
        ...
        <div key={"ital-row"} style={styles.row}>
          <div style={styles.prop}>italic</div>
          <div style={styles.val}>
            <Slider
              id="ital"
              min="0"
